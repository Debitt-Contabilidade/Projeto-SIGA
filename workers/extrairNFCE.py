...
def baixarNFCECompetencia(api_token,competencia,cnpj,codigo_dominio):
    ...
    for i in range(1, int(qtd_iteracoes)+1):
        ...
        try:
            link_consulta = f"https://siga.sefaz.ce.gov.br/api/v1/unidades/{cnpj}/documentos-fiscais/nfc-e?size=50&page={i}&sort=datEmissao,asc&dat-referencia={data_inicio},{data_fim}&tipo-operacao=SAIDA&resultado-processamento=AUTORIZADA"
            consulta = requests.get(link_consulta, headers=headers)
            json_data = consulta.json()
        ...
        pasta_json = Path("./temp/temp_json/temp_json_nfce_competencia")
        ...
        df_final.to_csv(f"./dados/nfce_competencia/nfce_competencia_{codigo_dominio}_{cnpj}_{competencia.replace("-","_")}.csv", index=False)
...
def main():
    ...
    dados = baixarNFCECompetencia(api_token,competencia,cnpj)
